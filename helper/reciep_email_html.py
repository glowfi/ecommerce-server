...
def paste_table(products, tax, shipping_fee, order_id, orderedAt, reciever, name):
    ...
    d = datetime.strptime(str(orderedAt), "%Y-%m-%d %H:%M:%S.%f")
    s = d.strftime("%m/%d/%Y %I:%M %p")

    global receipt_html_content

    receipt_html_content = receipt_html_content.replace(
        "[[PASTETABLEHERE]]", init_table
    )
    receipt_html_content = receipt_html_content.replace(
        "{{SubTotal}}", str(f"{round(amount,2)}")
    )
    receipt_html_content = receipt_html_content.replace("{{Tax}}", str(tax))
    receipt_html_content = receipt_html_content.replace(
        "{{Shipping Fee}}", str(shipping_fee)
    )
    receipt_html_content = receipt_html_content.replace(
        "{{total}}", str(f"{round(total,2)}")
    )
    receipt_html_content = receipt_html_content.replace(
        "{{receipt_id}}", "Order ID: " + str(order_id)
    )
    receipt_html_content = receipt_html_content.replace(
        "{{date}}", "Date of order: " + str(s)
    )
    receipt_html_content = receipt_html_content.replace(
        "[Product Name]", str(STORE_NAME)
    )
    receipt_html_content = receipt_html_content.replace("{{name}}", name)
    receipt_html_content = receipt_html_content.replace("{{support_url}}", "")
    receipt_html_content = receipt_html_content.replace(
